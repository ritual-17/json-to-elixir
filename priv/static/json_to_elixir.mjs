...
var JSON2 = class extends CustomType {
};
...
function view_switch_button() {
  return button(
    toList([class$("mb-auto"), on_click(new FlipInputMode())]),
...
function to_string2(language) {
  if (language instanceof JSON2) {
    return json_string;
  } else {
  ...
  return main(
    ...
    toList([
      div(
        toList([
          class$(
            "flex flex-row h-full w-full gap-10 overflow-y-auto"
          )
        ]),
        toList([
...
function view(model) {
  return body(
    toList([class$("flex flex-col h-dvh w-screen")]),
    toList([header2(), main_area(model)])
  );
}
function main2() {
